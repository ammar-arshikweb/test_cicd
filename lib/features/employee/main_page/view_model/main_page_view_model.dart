...
class MainPageViewModel extends ChangeNotifier {
  ...
  resetModel() async {
    ...
    selectedIndex = getSelectedIndex(); // Reset to default index
    // selectedIndex = 1; // Reset to default index
    _isSyncing = false;
  }

  static int getSelectedIndex() {
    has(String id) => Pref.getFunctionality()?.contains(id) ?? false;
    if (has(Constant.MOBILE_FEED)) return 0;
    if (has(Constant.MOBILE_ATTENDANCE)) return 1;
    if (has(Constant.MOBILE_AMC)) return 2;
    if (has(Constant.MOBILE_PROJECTS)) return 3;
    if (has(Constant.MOBILE_NOTIFICATION)) return 4;
    return 0;
  }
  ...
  Future<void> setupFCMTokenSync() async {
    ...
    // If APNS token is null, retry fetching it after a short delay
    if (apnsToken == null) {
      Log.e("APNS Token not available yet. Retrying...");
      await Future.delayed(Duration(seconds: 2));
      apnsToken = await _firebaseMessaging.getAPNSToken();
      if (apnsToken == null) {
        Log.e("APNS Token still not available after retry.");
      }
    }

    final currentToken = await _firebaseMessaging.getToken();
    Log.e("APNS FCM Token: $apnsToken");
    Log.e("Current FCM Token: $currentToken");
    final storedToken = Pref.getFcmToken();

    if (currentToken != null && currentToken != storedToken) {
      await MainPageRepo().storeFCMToken(fcmToken: currentToken);
      Pref.setFcmToken(currentToken);
    }

    FirebaseMessaging.instance.onTokenRefresh.listen((newToken) async {
      await MainPageRepo().storeFCMToken(fcmToken: newToken);
                    ...
                    children: [
                      ...
                      Center(
                        child: ElevatedButton(
                          ...
                          child: Text(
                            AppLocalizations.of(currentContext ?? context)?.close ?? '',
  ...
  Future<void> logoutButtonClick(BuildContext context) async {
    ...
    try {
      ...
      final apiRes = await MainPageRepo().logOut(logoutReq: logoutReq);
  ...
  Future<void> syncLocalData(BuildContext context) async {
    if (_isSyncing) return;
    try {
      _isSyncing = true;
      Log.e('Syncing local data...');
      context.showLoader();
      // Don't change the order of these sync methods
      await DatabaseService.syncClockInData();
      await DatabaseService.syncBreakInData();
      await DatabaseService.syncBreakOutData();
      await DatabaseService.syncClockOutData();
      await DatabaseService.syncEmergencyClockInData();
      await DatabaseService.syncEmergencyClockOutData();
      await DatabaseService.deleteEntriesOlderThanTwoDays();
    } catch (e) {
      Log.e('Error during syncLocalData: $e');
    } finally {
      _isSyncing = false;
      context.hideLoader();
    }
